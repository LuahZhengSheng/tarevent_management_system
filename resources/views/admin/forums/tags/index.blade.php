...
<script>
    ...
    const routes = {
    table: @json(route('admin.forums.tags.table')),
            approve: @json(route('admin.forums.tags.approve', ':id')),
            reject: @json(route('admin.forums.tags.reject', ':id')),
            update: @json(route('admin.forums.tags.update', ':id'))
    };
    ...
    const current = $('#paginationContainer .page-item.active .page-link').data('page') || 1;
            ...
            // ✅ 修改：data-id 改为 data-slug
            $(document).on('click', '.btn-tag-approve', function () {
    const slug = $(this).data('slug'); // 改这里
            const tagName = $(this).closest('tr').find('.user-name').text().trim();
            if (!confirm('Approve tag "' + tagName + '"?')) return;
            const url = routes.approve.replace(':id', slug); // 现在传递 slug
            ...
            url: url,
                    ...
                    console.error('Error details:', xhr);
                            alert(xhr.responseJSON?.message || "Approve failed");
                    }
            });
    });
            // ✅ 修改：data-id 改为 data-slug
    ...
    const slug = $(this).data('slug'); // 改这里
            const tagName = $(this).closest('tr').find('.user-name').text().trim();
            if (!confirm('Reject tag "' + tagName + '"? This will ban the tag.')) return;
            $.ajax({
            url: routes.reject.replace(':id', slug), // 传递 slug
                    type: "PATCH",
                    headers: {
                    "X-CSRF-TOKEN": csrfToken,
                            "X-Requested-With": "XMLHttpRequest"
                    ...
                    error: function(xhr) {
                    alert(xhr.responseJSON?.message || "Reject failed");
                    }
            });
    });
            // ✅ 修改：data-id 改为 data-slug
    ...
    const slug = $(this).data('slug'); // 改这里
            const tagName = $(this).closest('tr').find('.user-name').text().trim();
            if (!confirm('Disable tag "' + tagName + '"?')) return;
            $.ajax({
            url: routes.update.replace(':id', slug), // 传递 slug
                    type: "PATCH",
                    ...
                    error: function(xhr) {
                    alert(xhr.responseJSON?.message || "Disable failed");
                    }
            });
    });
            // ✅ 修改：data-id 改为 data-slug
    ...
    const slug = $(this).data('slug'); // 改这里
            const tagName = $(this).closest('tr').find('.user-name').text().trim();
            if (!confirm('Enable tag "' + tagName + '"?')) return;
            $.ajax({
            url: routes.update.replace(':id', slug), // 传递 slug
                    type: "PATCH",
                    data: { status: "active" },
                    ...
                    error: function(xhr) {
                    alert(xhr.responseJSON?.message || "Enable failed");
                    }
            });
    });
            // ✅ 修改：data-id 改为 data-slug
    ...
    e.preventDefault();
            ...
            url: routes.update.replace(':id', slug), // 传递 slug
                    ...
                    error: function(xhr) {
                    alert(xhr.responseJSON?.message || "Rename failed");
                    }
            });
    });
    })(jQuery);
