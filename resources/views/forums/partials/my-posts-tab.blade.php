...
<article class="post-item" data-post-id="{{ $post->id }}">
    ...
    <div class="post-media">
        <div class="media-gallery-facebook {{ $layoutClass }}">
            ...
            @if($index < 5)
            @php
            // 兼容：新 key + 旧 key（避免 Undefined array key）
            $type = $mediaItem['mediatype'] ?? ($mediaItem['type'] ?? 'image');
            $src  = $mediaItem['mediaurl'] ?? ($mediaItem['url'] ?? '');
            $thumbUrl = $mediaItem['thumbnailurl'] ?? ($mediaItem['thumbnail_url'] ?? $src);
